...
function LastFM(options){
	...
	var internalCall = function(params, callbacks, requestMethod) {
		currentCalls++;
		var t = function(calls) {
			return Math.floor(Math.random()*((calls-1)* 350));
		};
		setTimeout(function() {
			internalCallREAL(params, callbacks, requestMethod);
		}, t(currentCalls));
	};*/
	...
	var internalCall = function(params, callbacks, requestMethod){
		...
		/* Cross-domain GET request (JSONP). */
			...
			/* MY HACKS: retry on timeout! */

			// Let's cook up a function name that makes caching possible			
			var jsonp = 'jsonp';
			$.each(params, function(key, value) {
				// TODO: '->ZZZZ is not the best answer. make up a better one. testcase: tha_bisnas, 2010
				jsonp += (value+"").replace("'", "ZZZZ").replace( /\W/g , ""); // remove all non-word characters
			});

			/*
			var alarm = setTimeout(function() { // TODO: no idea how this would work on POST
												//       ...or at all?
				console.log("TIMEOUT!");
